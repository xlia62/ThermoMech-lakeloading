function [gridt,H,E,LEMpar] = surface_topography_FS(gridx,gridy,xstp1,ystp1,gridcx,gridcy,xstpc1,ystpc1,vx,vy,gridt,H,E,tstp,LEMpar,timestep)
...
vx = repmat(gridt(4,:)*yr2sec,LEMpar.ny,1);
vx = reshape(vx',1,LEMpar.nn);
vy = zeros(size(vx));
vx = LEMpar.NP.array(vx);
vy = LEMpar.NP.array(vy);
LEMpar.FS.fastscape_set_v(vx,vy);

% Note sinsce y is depth, uplift is negative of vy
u = repmat(-gridt(5,:)*yr2sec,LEMpar.ny,1);
u = reshape(u',1,LEMpar.nn);
u = LEMpar.NP.array(u);
LEMpar.FS.fastscape_set_u(u);
...
for ijk = 1:ntimes
    ...
    LEMpar.FS.fastscape_set_dt(dt_surf);
...
LEMpar.FS.fastscape_copy_lake_depth(LEMpar.lake_depth);
