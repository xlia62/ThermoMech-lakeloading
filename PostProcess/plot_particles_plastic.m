function output_image=plot_particles_plastic(input_dir,iframe,min_s,max_s, zoom_type, data_type)
% zoom type
...
if data_type == 1
    MGII = MGII;
    unit = {['Accumulated',newline,...
    'plastic strain (lg)']};
elseif data_type ==2
    MGII = sqrt(MEXX.*MEXX + MEXY.*MEXY);
    unit = {'Strain rate lg(1/s)'};  
end  
...
set(gca,'FontSize',25)
set(s, 'AlphaData', log10(markstrain) > log10(min_s), 'FaceAlpha',  'interp')  %'FaceAlpha',.3,'EdgeAlpha',.3
%set(s, 'FaceAlpha',.8,'EdgeAlpha',.3)
ylabel(c,[unit],'FontSize',25);
caxis([log10(min_s) log10(max_s)])
%caxis([threshold 10])
...
output_image = ['output/AfricaModels2022/Figure/extraction/',char(figname(end)),'_frame',...
    num2str(iframe), '_P_log_', num2str(min_s),'_',num2str(max_s), '_type', num2str(zoom_type), '_data_',...
    num2str(data_type)];
