function T=plot_t_meltV(inpath,nbegin, nfreq, nend,xlims,ylims, melt_i)
...
% calculate addition melt
diff_melt = zeros(length(melt), 1);
for j=1: length(melt)
    diff_melt(j) = melt_drop(j) - melt(j);
end
...
yyaxis left
% plot (time, melt,'linewidth',2);
% hold on;
% plot (time, melt_drop,'linewidth',2);
plot (time, diff_melt,'linewidth',2, 'Color', 'red');
xlabel('model time, (Myr)');
%ylabel('Melt volume, (km^2/km)');
if melt_i == 1
    ylabel('Differential melt volume, (km^3/km)');
elseif melt_i == 2
    ylabel('Differential extracted melt volume, (km^3/km)');
else 
    ylabel('wrong input');
end
% set(gca, 'YScale', 'log');
...
yyaxis right
plot (time, water,'linewidth',2, 'Color', 'b');
ylabel('Water mass, (kg)');
legend({'differential melt volume with lake level change',...
    'surface water mass'}, 'Location','southeast');
%legend({'melt volume reference','melt volume with lake level change',...
%     'surface water mass'}, 'Location','southeast');
...
output_image = ['output/AfricaModels2022/Figure/extraction/',char(figname(end)),...
    '_diff_', num2str(type),'_history_',num2str(nbegin), '-',...
    num2str(nend), '_dt_', num2str(nfreq), '_nomark_', num2str(marker_per_grid),...
