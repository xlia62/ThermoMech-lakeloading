...
% Fixed Internal boundary condtion (set at end of file)
internalbound = false;
...
k1 = find(gridy>=bot_ocrust2,1,'first');
k2 = find(gridy<=oceanLAB_young,1,'last');
k3 = find(gridx>=1400e3,1,'first');

vintx = -3.0/100/yr2sec;

bintern = zeros(8,4);
bintern(1,:)=[k3 -1 -1 -1];      % Horizontal position of vx nodes with prescrbed velocity (no susch condition if negative)
bintern(2,:)=k1;       % Min vertical position
bintern(3,:)=k2;       % Max vertical position
bintern(4,:)=vintx;      % Prescribed shortening velocity, m/s
bintern(5,:)=[k3 -1 -1 -1];      % Horizontal position of vy nodes with prescrbed velocity (no susch condition if negative)
bintern(6,:)=k1;       % Min vertical position
bintern(7,:)=k2;       % Max vertical position
