...
% -------------------------------------------------------------------------
MRHO(1,1)=1.2;             % standard density, kg/m^3
...
MFLOW(2,2)=1e+19;           % viscosity, Pa s
...
% -------------------------------------------------------------------------
MRHO(5,1)=3200;             % standard density, kg/m^3
...
% MFLOW(15,2)= 2.5e+4;          % AD, 1/s/MPa^n
% MFLOW(15,3)=3.5;             % n
% MFLOW(15,4)=532;             % Ea, kJ/mol
% MFLOW(15,5)=8;               % Va, cm^3
...
MPHASE(5,1:2) = 225;             % Density Change (kg/m^3)
...
plate_age_old = 70e6*yr2sec;  % Subducting

% Age of Oceanic Plate (s)
plate_age_young = 5e6*yr2sec; % Overriding
...
% bot_ccrust1_right = water_lev + upper_cont_thick_right;
...
oceanic_lith_thick_young = 22e3;
oceanic_lith_thick_old = 62e3;
...
oceanLAB_old = sticky_layer + sed_thick + upper_ocrust_thick + lower_ocrust_thick + oceanic_lith_thick_old;
oceanLAB_young = sticky_layer + sed_thick + upper_ocrust_thick + lower_ocrust_thick + oceanic_lith_thick_young;
...
for mm1 = 1:marknum
    ...
    %--------------------------------------------------------
    % Overriding Young Ocean Plate (left)
    %--------------------------------------------------------
    ...
    % Basalt in the gabbro layer
    if (MX(mm1) > TrenchX_0 && MX(mm1) <= (TrenchX + (bot_ocrust2 - MY(mm1)) * 10) && MY(mm1) > bot_ocrust1 && MY(mm1) <= bot_ocrust2)
        MI(mm1) = 4;
    end
    
    % Hydrate mantle
    if (MX(mm1) > TrenchX_0 && MX(mm1) < TrenchX && MY(mm1) > bot_ocrust2 && MY(mm1) <= oceanLAB_old-20e3)
        MI(mm1) = 11;
...
%     end
    ...
    if( MY(mm1) > sticky_layer && MY(mm1) < 100e3 && MX(mm1) < TrenchX)
       z = MY(mm1) - sticky_layer;
       t100 = tbottom-tgrad*(ysize - 100e3);    % T of asthenosphere at y=100 km
       age=5e+6*(365.25*24*3600);  % Plate age, s
       %MTK(mm1) = t100 + (ttop-t100)*(1-erf(z/(2*sqrt(kappa*plate_age_young))));
       MTK(mm1)=t100+(ttop-t100)*(1-erf((MY(mm1)-10000)/2/(kappa*age)^0.5));
    end
    
    % Oceanic geotherm for subducting plate of specified age
    % after Turcotte & Schubert (2002)
    if( MY(mm1) > sticky_layer && MY(mm1) < 200e3 && MX(mm1) > TrenchX)   
        z = MY(mm1) - sticky_layer;
        t200 = tbottom-tgrad*(ysize - 200e3);    % T of asthenosphere at y=200 km
        age=7e+7*(365.25*24*3600);  % Plate age, s
        %MTK(mm1) = t200 + (ttop-t200)*(1-erf(z/(2*sqrt(kappa*plate_age_old))));
