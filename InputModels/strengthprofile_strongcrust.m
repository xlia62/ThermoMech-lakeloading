...
% gravity
lambda = 0; % ratio of pore-fluid pressure to lithostatic pressure ~= 0.4
g = 9.81;
R = 8.314;    % Gas constant
eii = S.eii;
factor= 10; % strong crust factor
% 1st layer; sediments
if t1 > 0
    k = S.z <= t1;
    z = S.z(k);
...
F = [o1; o2*factor;o3*factor;o4; o5];
...
S.syield = C0 + F0.*S.P;  % Pa
sigd_tension = -2*(C0+F0.*S.P*(1-lambda))./(sqrt(F0.^2+1)+F0);
sigd_compression = 2*(C0+F0.*S.P*(1-lambda))./(sqrt(F0.^2+1)-F0);

sii = F.* ((eii./Ad).^(1./n)).*exp((Ea*1000.+Va.*S.P*1e-6)./(n*R.*S.T)); % MPa

% The case of constant viscosity
k = n == 0;
sii(k) = 2*eii*Ad(k);

S.sigd_tension = max(-sii*1e6,sigd_tension); % Pa
S.sigd_compression = min(sii*1e6,sigd_compression); % Pa
