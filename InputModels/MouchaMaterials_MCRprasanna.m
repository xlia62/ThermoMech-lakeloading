...
MFLOW(1,1) = 0;               % -1= constant viscosity
MFLOW(1,2) = 1e+19;           % viscosity, Pa s
MFLOW(1,3) = 1;
...
MKT(4,2) = 474;               % a, W/m
MKT(4,3) = 4e-11;             % Pressure dependance of thermal conductivity b, W/m/Pa
...
MKT(6,2) = 807;               % a, W/m
MHR(6,1) = 1.14e-6;              % radiogenic heat production, W/m^3
...
MRHO(7,2) = 3e-5;             % thermal expansaxiion, 1/K
...
MKT(7,2) = 474;               % a, W/m
MHR(7,1) = 2.0e-7;              % radiogenic heat production, W/m^3
...
MFLOW(9,5) = 1.6;              % Va, cm^3 = = = > *1e-6 for J/Pa, or *0.1 for J/bar
                              % uni-axial compression % Lab to model factor                            
n = MFLOW(9,3);                            
MFLOW(9,6) = F(n);
...
MHR(9,1) = 3.5e-9;             % radiogenic heat production, W/m^3
...
% -------------------------------------------------------------------------
MRHO(11,1) = 2700;             % standard density, kg/m^3
...
MFLOW(11,2) = 3.2e-4;          % AD, 1/s/MPa^n
MFLOW(11,3) = 2.3;             % n
MFLOW(11,4) = 154;             % Ea, kJ/mol
MFLOW(11,5) = 0;               % Va, cm^3 = = = > *1e-6 for J/Pa, or *0.1 for J/bar
                            % uni-axial compression % Lab to model factor
n = MFLOW(11,3);                            
MFLOW(11,6) = F(n); 

MMU(11,1) = 1e+11;               % shear modulus, Pa
MPL(11,1) = 1e+6;              % C0, Pa
MPL(11,2) = 1e+6;              % C1, Pa
MPL(11,3) = 0.3;                 % sin(FI0)
MPL(11,4) = 0.3;                 % sin(FI1)
MPL(11,5) = 0;                   % GAM0
MPL(11,6) = 0.25;                % GAM1
...
% -------------------------------------------------------------------------
%% 14 Plume material tracking equivlent to Asthenosphere = 10
% -------------------------------------------------------------------------
...
MFLOW(15,5) = 5;               % Va, cm^3 = = = > *1e-6 for J/Pa, or *0.1 for J/bar
                            % uni-axial compression % Lab to model factor
n = MFLOW(15,3);                            
MFLOW(15,6) = F(n);  
