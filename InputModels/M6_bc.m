...
% Remove external boundary condition
remove_time = false;

% Gerya-style moving grid (keep grid centered on trench)
moving_trench = false;
regridx = false;    % had been true
...
% mass
if moving_grid || moving_trench
    ytopfixed = true;
    ybotfixed = true;
    vytop = sticky_layer*(vxright-vxleft)/xsize;
    vybottom = (vxright-vxleft)*(sticky_layer-ysize)/xsize;
end
...
% Vertical position of "mobile wall" (m)
min_vert_m = sticky_layer;
%oceanLAB_young = 80e3;
max_vert_m = 150e3;

% Horizontal position of wall (m)
global ridge_loc
hor_m = ridge_loc; % (M6)
...
% Convert vertical position to node number
% Convert velocity to m/s
tmp_min = abs(G.gridy-min_vert_m);
[~,min_vert_node] = min(tmp_min);
tmp_max = abs(G.gridy-max_vert_m);
[~,max_vert_node] = min(tmp_max);
tmp_hor = abs(G.gridx-hor_m);
[~,hor_node] = min(tmp_hor);
con_velo = wall_vel/yr2sec/100;
    
bintern = zeros(8,4);
%bintern(1,:)=[hor_node-3 hor_node-2 hor_node-1 hor_node];      % Horizontal position of vx nodes with prescrbed velocity (no susch condition if negative)
bintern(1,:)=hor_node;      % Horizontal position of vx nodes with prescrbed velocity (no susch condition if negative)
bintern(2,:)=min_vert_node;       % Min vertical position
bintern(3,:)=max_vert_node;       % Max vertical position
bintern(4,:)=con_velo;      % Prescribed shortening velocity, m/s
