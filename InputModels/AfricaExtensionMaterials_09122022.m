...
% -------------------------------------------------------------------------
%% Material properties used for Africa Extension Models
%
% Rock properties taken from Theunissen and Ritske, 2019 , except for the
% following:
%
% Continental Lithospheric Mantle is assumed to be dry since African cratons are dry (Katayama and Korenaga, 2011 GSA Sp. Pr. 478)
% 
% Rheology of lithospheric mantle and upper mantle is taken from Korenaga and Karato (2008) doi.org/10.1029/2007JB005100
%
% Mantle conductivity is fixed to 3.5 W/m/K and specfic heat capacity is
% taken from Gerya 2010 for all rocks at 1000 J/kg/K
% -------------------------------------------------------------------------

% Standard 19, Addons must start after 19 below
ntype = 19;
...
MatProp{11} = 'Hydrated Upper Mantle';
...
% Add-ons HERE! But you will need to also edit the geodyn_cmap and add the
% colors.

n_type = length(MatProp);

% MRHO  = density (kg/m3): RHO*[1-ALP*(T-273)]*[1+BET*(P-1e+5)], includes
%         melt density
% MFLOW = power-law: EPSILONii= AD*SIGMAii^n*exp[-(Ea+Va*P)/RT) !! computed
%                                       from stress to avoid elastic strain
% MMU = shear modulus (Pa), setting viscoelastic to false will increase the
%       shear modulus to high values for effective rheology calculations
%       essentially removing the contributions to elastic strains.
...
% Initialize MFLOW, MPL
MFLOW = zeros(n_type,6);
MPL = zeros(ntype,7);

%-----------------------------------------
% !!! Is not used in ThermoMech2D.m  !!!

% Dummy values for diffusion creep to yield a very high viscosity in
% such that it will not contribute to composite viscosity that is a
% based on the reciprical (harmonic) average.
...
% materials! Done as an inline function. Can be used to increase or
% decrease (tune) the viscosity as well.
F = @(n) 1;  % Constant
...
%% 3 = Sediments
% -------------------------------------------------------------------------
MRHO(3,1) = 2640;             % standard density, kg/m^3
MRHO(3,2) = 3.1e-5;             % thermal expansion, 1/K
...
MKT(3,1) = 2.25;              % k0, W/m/K
MKT(3,2) = 0;                 % a, W/m <-- enter 0 for constant (see above)
MHR(3,1) = 1.196e-6;          % radiogenic heat production, W/m^3
MHE(3,1) = 1;                 % shear and adiobatic heating efficiency

% -------------------------------------------------------------------------
%% 4 = Upper oceanic crust basalt
% -------------------------------------------------------------------------
...
MPL(4,1) = 1e+6;              % C0, Pa
MPL(4,2) = 1e+6;              % C1, Pa
MPL(4,3) = 0.03;                 % sin(FI0)
MPL(4,4) = 0.03;                 % sin(FI1)
MPL(4,5) = 0;                   % GAM0
MPL(4,6) = 0.1;                 % GAM1
...
% -------------------------------------------------------------------------
MRHO(5,1) = 3000-150;             % standard density, kg/m^3
...
MPL(6,1) = 20e+6;              % C0, Pa
MPL(6,2) = 4e+6;              % C1, Pa
MPL(6,3) = 0.259;                 % sin(FI0)
MPL(6,4) = 0.035;                 % sin(FI1)
MPL(6,5) = 0;                 % GAM0
MPL(6,6) = 1;                 % GAM1
MPL(6,7) = 200e6;               % Maximum yield strength, Pa
MCP(6,1) = 1000;                % Cp, J/kg
MKT(6,1) = 2.25;              % k0, W/m/K
MKT(6,2) = 0;                  % a, W/m
MHR(6,1) = 1.6e-6;              % radiogenic heat production, W/m^3
...
MKT(7,1) = 2.25;              % k0, W/m/K
MKT(7,2) = 0;               % a, W/m
MHR(7,1) = 2.5e-7;              % radiogenic heat production, W/m^3
...
% -------------------------------------------------------------------------
%% 9 = Continental Lithospheric mantle -- dry olivine
% -------------------------------------------------------------------------
...
MRHO(9,5) = 0;               % depletion (compositional density constrast) kg/m^3

MFLOW(9,1) = 1;              % 1= power law (dry olivine: Kawazoe et al, 2009)
% Dislocation creep
MFLOW(9,2) = 1e+6;    % AD, 1/s/MPa^n
MFLOW(9,3) = 5;             % n
MFLOW(9,4) = 610;             % Ea, kJ/mol
MFLOW(9,5) = 13;              % Va, cm^3 = = = > *1e-6 for J/Pa, or *0.1 for J/bar
                              % uni-axial compression % Lab to model factor                            
n = MFLOW(9,3);                            
MFLOW(9,6) = F(n);
...
% MFLOW(9,8) = 3e-45;           % AD, 1/s/MPa^n (Garelle et al 2014)
% MFLOW(9,9) = 1;              % n
...
% Disclocation creep
MFLOW(10,1) = 1;               % 1= power law (dry olivine: Kawazoe et al, 2009)
MFLOW(10,2) = 1e+6;          % AD, 1/s/MPa^n
MFLOW(10,3) = 5;             % n
MFLOW(10,4) = 580;             % Ea, kJ/mol   (Lower end (610+/-30)
MFLOW(10,5) = 10;              % Va, cm^3 = = = > *1e-6 for J/Pa, or *0.1 for J/bar (lower end 13+/-8)
                              % uni-axial compression % Lab to model factor                            
n = MFLOW(10,3);                            
MFLOW(10,6) = F(n);  
...
% MFLOW(10,8) = 3e-5;           % AD, 1/s/MPa^n (Garelle et al 2014)
% MFLOW(10,9) = 1;              % n
...
MMU(10,1) = 74e+9;           % shear modulus, Pa
MPL(10,1) = 20e+6;           % C0, Pa
MPL(10,2) = 4e+6;              % C1, Pa
MPL(10,3) = 0.259;             % sin(FI0) friction coefficient
MPL(10,4) = 0.035;             % sin(FI1)
MPL(10,5) = 0;                 % GAM0
MPL(10,6) = 1;                 % GAM1
MPL(10,7) = 200e6;             % Maximum yield strength, Pa
MCP(10,1) = 1000;              % Cp, J/kg
MKT(10,1) = 3.5;              % k0, W/m/K
MKT(10,2) = 0;                 % a, W/m
MKT(10,3) = 0;                 % Pressure dependance of thermal conductivity b, W/m/Pa
MHR(10,1) = 2.2e-8;              % radiogenic heat production, W/m^3
MHE(10,1) = 1;                 % shear and adiobatic heating efficiency

% -------------------------------------------------------------------------
%% 11 = Hydrated Upper Mantle
% -------------------------------------------------------------------------
MRHO(11,1) = 3300;             % standard density, kg/m^3
MRHO(11,2) = 3e-5;             % thermal expansion, 1/K
MRHO(11,3) = 1e-11;            % compressibility, 1/Pa
MRHO(11,4) = 2900;             % melt density, kg/m^3
MFLOW(11,1) = 1;               % 1= power law
MFLOW(11,2) = 10*640000;        % AD, 1/s/MPa^n multiplying by COH2 800 ppm H/Si with r=2
MFLOW(11,3) = 3.5;             % n
MFLOW(11,4) = 523;          % Ea, kJ/mol
MFLOW(11,5) = 4;               % Va, cm^3 = = = > *1e-6 for J/Pa, or *0.1 for J/bar
                            % uni-axial compression % Lab to model factor
n = MFLOW(11,3);                            
MFLOW(11,6) = F(n); 

% Diffusion creep        
% MFLOW(11,7) = 1;               % 1= power law (dry olivine: Kawazoe et al, 2009)
% MFLOW(11,8) = 3e-5;           % AD, 1/s/MPa^n (Garelle et al 2014)
% MFLOW(11,9) = 1;              % n
% MFLOW(11,10) = 300;            % Ea, kJ/mol
% MFLOW(11,11) = 4;             % Va, cm^3 = = = > *1e-6 for J/Pa, or *0.1 for J/bar
% n = MFLOW(11,9);                            
% MFLOW(11,12) = F(n);

MMU(11,1) = 36e+9;               % shear modulus, Pa
MPL(11,1) = 20e+6;              % C0, Pa
MPL(11,2) = 4e+6;              % C1, Pa
MPL(11,3) = 0.259;                 % sin(FI0)
MPL(11,4) = 0.035;                 % sin(FI1)
MPL(11,5) = 0;                   % GAM0
MPL(11,6) = 1;                   % GAM1
MPL(11,7) = 200e6;               % Maximum yield strength, Pa
MCP(11,1) = 1000;                % Cp, J/kg
MKT(11,1) = 3.5;              % k0, W/m/K
MKT(11,2) = 0;               % a, W/m
MKT(10,3) = 0;                 % Pressure dependance of thermal conductivity b, W/m/Pa
MHR(11,1) = 1.0e-6;              % radiogenic heat production, W/m^3
MHE(11,1) = 1;                 % shear and adiobatic heating efficiency

% -------------------------------------------------------------------------
%% 12 Plume material tracking equivlent to Asthenosphere = 10
% -------------------------------------------------------------------------

MRHO(12,1) = 3300;             % standard density, kg/m^3
MRHO(12,2) = 3e-5;             % thermal expansion, 1/K
MRHO(12,3) = 1e-11;            % compressibility, 1/Pa
MRHO(12,4) = 2900;             % melt density, kg/m^3
MRHO(12,5) = 10;               % compositianal density, kg/m^3
% Disclocation creep
MFLOW(12,1) = 1;               % 1= power law (dry olivine: Kawazoe et al, 2009)
MFLOW(12,2) = 1e+6;          % AD, 1/s/MPa^n
MFLOW(12,3) = 5;             % n
MFLOW(12,4) = 580;             % Ea, kJ/mol   (Lower end (610+/-30)
MFLOW(12,5) = 10;              % Va, cm^3 = = = > *1e-6 for J/Pa, or *0.1 for J/bar (lower end 13+/-8)
                              % uni-axial compression % Lab to model factor                            
n = MFLOW(12,3);                            
MFLOW(12,6) = F(n);  
...
MMU(12,1) = 74e+9;           % shear modulus, Pa
MPL(12,1) = 20e+6;           % C0, Pa
MPL(12,2) = 4e+6;              % C1, Pa
MPL(12,3) = 0.259;             % sin(FI0) friction coefficient
MPL(12,4) = 0.035;             % sin(FI1)
MPL(12,5) = 0;                 % GAM0
MPL(12,6) = 1;                 % GAM1
MPL(12,7) = 200e6;             % Maximum yield strength, Pa
MCP(12,1) = 1000;              % Cp, J/kg
MKT(12,1) = 3.5;              % k0, W/m/K
MKT(12,2) = 0;                 % a, W/m
MKT(12,3) = 0;                 % Pressure dependance of thermal conductivity b, W/m/Pa
MHR(12,1) = 2.2e-8;              % radiogenic heat production, W/m^3
MHE(12,1) = 1;                 % shear and adiobatic heating efficiency

% -------------------------------------------------------------------------
%% 13 = Eclogite (Use Gabbro Rheology instead of basalt)
% -------------------------------------------------------------------------
MRHO(13,1) = 3400;             % standard density, kg/m^3
MRHO(13,2) = 3e-5;             % thermal expansion, 1/K
MRHO(13,3) = 1e-11;            % compressibility, 1/Pa
MRHO(13,4) = 2900;             % melt density, kg/m^3
MFLOW(13,1) = 1;               % 1= power law (plagioclase An75: Ranalli, 1995)
MFLOW(13,2) = 3.3e-4;          % AD, 1/s/MPa^n
MFLOW(13,3) = 3.2;             % n
MFLOW(13,4) = 480;             % Ea, kJ/mol
...
MPL(13,1) = 1e+6;              % C0, Pa
MPL(13,2) = 1e+6;              % C1, Pa
MPL(13,3) = 0.6;               % sin(FI0), strong
MPL(13,4) = 0.3;               % sin(FI1), weak
MPL(13,5) = 0;                 % GAM0
MPL(13,6) = 1;                 % GAM1
...
MFLOW(14,1) = 1;            % 1= power law ( after Garell et al, 2014)
MFLOW(14,2) = 1e-21;        % AD, 1/s/MPa^n to convert from Pa^n, (1e-6*AD^(-1/n))^-n
MFLOW(14,3) = 3.5;             % n
MFLOW(14,4) = 530;             % Ea, kJ/mol
MFLOW(14,5) = 15;              % Va, cm^3 = = = > *1e-6 for J/Pa, or *0.1 for J/bar
                              % uni-axial compression % Lab to model factor  
n = MFLOW(14,3);
MFLOW(14,6) = F(n);            % uni-axial compression % Lab to model factor
...
MPL(14,1) = 2e+6;            % C0, Pa
MPL(14,2) = 2e+6;             % C1, Pa
MPL(14,3) = 0.2;               % sin(FI0)
MPL(14,4) = 0.2;               % sin(FI1)
MPL(14,5) = 0;                 % GAM0
MPL(14,6) = 1;                 % GAM1
MPL(14,7) = 100e6;             % Maximum yield strength, Pa
...
% -------------------------------------------------------------------------
%% 16 = Upper oceanic crust plateau (basalts)
% -------------------------------------------------------------------------
MRHO(16,:) = MRHO(4,:);
MFLOW(16,:) = MFLOW(4,:);

MMU(16,1) = MMU(4,1);             % shear modulus, Pa
MPL(16,:) = MPL(4,:);
MCP(16,1) = MCP(4,1);                % Cp, J/kg
MKT(16,:) = MKT(4,:);
MHR(16,1) = MHR(4,1);              % radiogenic heat production, W/m^3
MHE(16,1) = MHE(4,1);                 % shear and adiobatic heating efficiency

% -------------------------------------------------------------------------
%% 17 = Lower oceanic crust plateau
% -------------------------------------------------------------------------

MRHO(17,:) = MRHO(5,:);
MFLOW(17,:) = MFLOW(5,:);

MMU(17,1) = MMU(5,1);             % shear modulus, Pa
MPL(17,:) = MPL(5,:);
MCP(17,1) = MCP(5,1);                % Cp, J/kg
MKT(17,:) = MKT(5,:);
MHR(17,1) = MHR(5,1);              % radiogenic heat production, W/m^3
MHE(17,1) = MHE(5,1);                 % shear and adiobatic heating efficiency

% -------------------------------------------------------------------------
%% 18 = Upper continental crust (granodiorite) stripe
% -------------------------------------------------------------------------
MRHO(18,:) = MRHO(6,:);
MFLOW(18,:) = MFLOW(6,:);

MMU(18,1) = MMU(6,1);             % shear modulus, Pa
MPL(18,:) = MPL(6,:);
MCP(18,1) = MCP(6,1);                % Cp, J/kg
MKT(18,:) = MKT(6,:);
MHR(18,1) = MHR(6,1);              % radiogenic heat production, W/m^3
MHE(18,1) = MHE(6,1);                 % shear and adiobatic heating efficiency


% -------------------------------------------------------------------------
%% 19 = Lower continental crust (diorite) stripe
% -------------------------------------------------------------------------
MRHO(19,:) = MRHO(7,:);
MFLOW(19,:) = MFLOW(7,:);

MMU(19,1) = MMU(7,1);             % shear modulus, Pa
MPL(19,:) = MPL(7,:);
MCP(19,1) = MCP(7,1);                % Cp, J/kg
MKT(19,:) = MKT(7,:);
MHR(19,1) = MHR(7,1);              % radiogenic heat production, W/m^3
MHE(19,1) = MHE(7,1);                 % shear and adiobatic heating efficiency
