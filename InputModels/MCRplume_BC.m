...
v0 = 17;
d0 = 120e3;
d1 = 100e3; % <-- ramp down distance

% Internal nodes for vx, ghost nodes are added
yi = [G.gridy(1)-d1 ; (G.gridy(1:end-1) + G.gridy(2:end))/2 ; G.gridy(end)+d1];
k1 = find(yi <= d0);
k2 = find(yi > d0 & yi <= d0+d1);

vxleft = zeros(ynum+1,1);
vxright = zeros(ynum+1,1);

vxleft(k1) = v0; 
vxright(k1) = v0;

slope = -v0/d1;
intercept = v0 - slope*d0;

vxleft(k2) = slope*yi(k2) + intercept;
vxright(k2) = slope*yi(k2) + intercept;
