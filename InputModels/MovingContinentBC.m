...
% move fine grid region
move_fine = false;
vf = -2.0;  % cm/yr
vf = vf/100/yr2sec;  % convert to m/s

% Generate new grid in the x-direction
regridx = false;
...
% Stop forced boundary conditon after Myr (if 0, then not removed).
remove_time = false;
...
% Set up function for velocity boundary condition
v0 = 10.0;  % cm/yr
d0 = 200e3;   %150 --> 170 to 250
d1 = 280e3;
m = v0/(d0-d1);
b = -v0*d1/(d0-d1);

vxleft = zeros(ynum+1,1);
vright = zeros(ynum+1,1);

vxleft(gridy<=sticky_layer) =v0;
vxleft(gridy>sticky_layer) = v0;
k = gridy>=d0;

vxleft(k) = m*gridy(k) + b;
vxleft(gridy>d1) = 0;
vxright = vxleft;
...
if moving_grid
    ...
    vybot = (vxright-vxleft)*(sticky_layer-ysize)/xsize;
end
