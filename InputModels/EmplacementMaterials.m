...
% -------------------------------------------------------------------------
MRHO(1,1) = 1000;%1.22;             % standard density, kg/m^3
...
MFLOW(2,2) = 1e+13;           % viscosity, Pa s
...
% -------------------------------------------------------------------------
%% 3 = Sediments ==> Upper continental crust (granodiorite) 1
% -------------------------------------------------------------------------
MRHO(3,1) = 2700;             % standard density, kg/m^3
...
MPL(3,5) = 0;                 % GAM0
MPL(3,6) = 0.1;                 % GAM1
MPL(3,7) = 200e9;               % Maximum yield strength, Pa 
...
% -------------------------------------------------------------------------
%% 4 = Upper oceanic crust (basalt) --> UC 1
% -------------------------------------------------------------------------
MRHO(4,1) = 2700;             % standard density, kg/m^3 --> was 3000
...
MMU(4,1) = 1e+10;             % shear modulus, Pa --> was 2.5e10
MPL(4,1) = 1e+6;              % C0, Pa
MPL(4,2) = 1e+6;              % C1, Pa
MPL(4,3) = 0.2;                 % sin(FI0) --> was 0
MPL(4,4) = 0.0;                 % sin(FI1)
MPL(4,5) = 0;                 % GAM0
MPL(4,6) = 0.1;                 % GAM1
MPL(4,7) = 200e9;               % Maximum yield strength, Pa 
MCP(4,1) = 1000;                % Cp, J/kg
MKT(4,1) = 0.64;              % k0, W/m/K --> as 1.18
MKT(4,2) = 807;               % a, W/m --> was 474
MHR(4,1) = 1.0e-6;            % radiogenic heat production, W/m^3 --> was 2.5e-7
...
MKT(5,2) = 474;               % a, W/m
MKT(5,3) = 4e-11;             % Pressure dependance of thermal conductivity b, W/m/Pa
MHR(5,1) = 2.5e-7;              % radiogenic heat production, W/m^3
MHE(5,1) = 1;                 % shear and adiobatic heating efficiency

% -------------------------------------------------------------------------
%% 6 = Upper continental crust (granodiorite) 2
% -------------------------------------------------------------------------
...
MPL(6,5) = 0;                 % GAM0
MPL(6,6) = 0.1;                 % GAM1
MPL(6,7) = 200e9;               % Maximum yield strength, Pa 
MCP(6,1) = 1000;                % Cp, J/kg
MKT(6,1) = 0.64;              % k0, W/m/K
MKT(6,2) = 807;               % a, W/m
MHR(6,1) = 1.0e-6;              % radiogenic heat production, W/m^3 --> was 2.0e-6
MHE(6,1) = 1;                 % shear and adiobatic heating efficiency

% -------------------------------------------------------------------------
%% 7 = Lower continental crust (diorite)
% -------------------------------------------------------------------------
MRHO(7,1) = 2700;             % standard density, kg/m^3
...
n = MFLOW(7,3);                            
MFLOW(7,6) = F(n);  
MMU(7,1) = 1e+10;             % shear modulus, Pa
MPL(7,1) = 1e+6;              % C0, Pa
MPL(7,2) = 1e+6;              % C1, Pa
MPL(7,3) = 0.2;               % sin(FI0)
MPL(7,4) = 0.0;               % sin(FI1)
MPL(7,5) = 0;                 % GAM0
MPL(7,6) = 0.1;                 % GAM1
MPL(7,7) = 200e9;               % Maximum yield strength, Pa
MCP(7,1) = 1000;                % Cp, J/kg
MKT(7,1) = 0.64;              % k0, W/m/K
MKT(7,2) = 474;               % a, W/m
...
MRHO(9,4) = 2700;            % melt density, kg/m^3   (Gerya and Mellick, 2010)
...
MPL(9,1) = 10e+6;              % C0, Pa --> changed 1e6
MPL(9,2) = 3e+6;              % C1, Pa --> changed 1e6
MPL(9,3) = 0.6;                % sin(FI0)
MPL(9,4) = 0.0;                % sin(FI1)
MPL(9,5) = 0;                  % GAM0
MPL(9,6) = 0.1;                  % GAM1
MPL(9,7) = 200e9;               % Maximum yield strength, Pa 
MCP(9,1) = 1000;               % Cp, J/kg
MKT(9,1) = 0.73;               % k0, W/m/K  (Gerya and Mellick, 2010)
MKT(9,2) = 1293;               % a, W/m
MKT(9,3) = 0;                  % Pressure dependance of thermal conductivity b, W/m/Pa
...
MRHO(10,4) = 2700;             % melt density, kg/m^3
...
MFLOW(10,5) = 10;              % Va, cm^3 = = = > *1e-6 for J/Pa, or *0.1 for J/bar
                              % uni-axial compression % Lab to model factor                            
n = MFLOW(10,3);                            
MFLOW(10,6) = F(n);  
...
MFLOW(10,8) = 3e-45;           % AD, 1/s/MPa^n (Garelle et al 2014)
MFLOW(10,9) = 1;              % n
...
MPL(10,1) = 1e+6;              % C0, Pa
MPL(10,2) = 1e+6;              % C1, Pa
MPL(10,3) = 0.6;               % sin(FI0) friction coefficient
MPL(10,4) = 0;                 % sin(FI1)
MPL(10,5) = 0;                 % GAM0
MPL(10,6) = 0.1;                 % GAM1
MPL(10,7) = 200e9;               % Maximum yield strength, Pa 
...
% -------------------------------------------------------------------------
%% 11 = Magmatic source region (Hydrated Mantle)
% -------------------------------------------------------------------------

MRHO(11,1) = 3300;             % standard density, kg/m^3
MRHO(11,2) = 3e-5;             % thermal expansion, 1/K
MRHO(11,3) = 1e-11;            % compressibility, 1/Pa
MRHO(11,4) = 2700;             % melt density, kg/m^3   (Gerya and Mellick, 2010)
MRHO(11,5) = 0;            % depletion (compositional density constrast) kg/m^3
...
MFLOW(11,8) = 1e-45;            % AD, 1/s/MPa^n (unknown)
MFLOW(11,9) = 1;              % n
MFLOW(11,10) = 335;            % Ea, kJ/mol
...
MPL(11,5) = 0;                 % GAM0
MPL(11,6) = 0.1;                 % GAM1
MPL(11,7) = 100e6;               % Maximum yield strength, Pa
MCP(11,1) = 1000;              % Cp, J/kg
MKT(11,1) = 0.73;              % k0, W/m/K
MKT(11,2) = 1293;              % a, W/m
MKT(11,3) = 0;             % Pressure dependance of thermal conductivity b, W/m/Pa
...
MPL(12,1) = 1e+6;              % C0, Pa
MPL(12,2) = 1e+6;              % C1, Pa
MPL(12,3) = 0.0;                 % sin(FI0)
MPL(12,4) = 0.0;                 % sin(FI1)
MPL(12,5) = 0;                 % GAM0
MPL(12,6) = 0.1;                 % GAM1
MPL(12,7) = 200e9;               % Maximum yield strength, Pa
...
MPL(14,3) = 0.6;               % sin(FI0) friction coefficient
MPL(14,4) = 0.3;               % sin(FI1)
MPL(14,5) = 0;                 % GAM0
MPL(14,6) = 1;                 % GAM1
MPL(14,7) = 200e9;               % Maximum yield strength, Pa
...
% -------------------------------------------------------------------------
%% 18 = Upper continental crust (granodiorite) -- weak faulting
% -------------------------------------------------------------------------
MRHO(18,1) = 2700;             % standard density, kg/m^3
MRHO(18,2) = 3e-5;             % thermal expansion, 1/K
MRHO(18,3) = 1e-11;            % compressibility, 1/Pa
MRHO(11,4) = 2400;             % melt density, kg/m^3
MFLOW(18,1) = 1;               % 1= power law (wet quartzite: Ranalli, 1995)
MFLOW(18,2) = 3.2e-4;          % AD, 1/s/MPa^n
MFLOW(18,3) = 2.3;             % n
MFLOW(18,4) = 154;             % Ea, kJ/mol
MFLOW(18,5) = 0;               % Va, cm^3 = = = > *1e-6 for J/Pa, or *0.1 for J/bar
                            % uni-axial compression % Lab to model factor
n = MFLOW(18,3);                            
MFLOW(18,6) = F(n); 

MMU(18,1) = 1e+11;               % shear modulus, Pa
MPL(18,1) = 1e+6;              % C0, Pa
MPL(18,2) = 1e+6;              % C1, Pa
MPL(18,3) = 0.3;                 % sin(FI0)
MPL(18,4) = 0.3;                 % sin(FI1)
MPL(18,5) = 0;                   % GAM0
MPL(18,6) = 0.25;                % GAM1
MPL(18,7) = 200e6;               % Maximum yield strength, Pa
MCP(18,1) = 1000;                % Cp, J/kg
MKT(18,1) = 0.64;              % k0, W/m/K
MKT(18,2) = 807;               % a, W/m
MHR(18,1) = 1.0e-6;              % radiogenic heat production, W/m^3
MHE(18,1) = 1;                 % shear and adiobatic heating efficiency
