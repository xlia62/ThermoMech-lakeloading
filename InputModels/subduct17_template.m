...
% 16 Upper Mantle
% -------------------------------------------------------------------------
MRHO(16,1)=3300;             % standard density, kg/m^3
MRHO(16,2)=3e-5;             % thermal expansion, 1/K
MRHO(16,3)=1e-11;            % compressibility, 1/Pa
MRHO(16,4)=2900;             % melt density, kg/m^3
MRHO(16,5)= -20;             % compositional density constrast, kg/m^3
...
MPL(16,1)=10e+6;              % C0, Pa
MPL(16,2)=10e+6;              % C1, Pa
MPL(16,3)=0.6;               % sin(FI0)
MPL(16,4)=0.6;               % sin(FI1)
MPL(16,5)=0;                 % GAM0  : Accumulated strain regime
MPL(16,6)=1;                 % GAM1  : Accumulated strain regime
MCP(16,1)=1000;                % Cp, J/kg
MKT(16,1)=0.73;              % k0, W/m/K
MKT(16,2)=1293;              % a, W/m
MHR(16,1)=2.2e-8;              % radiogenic heat production, W/m^3% 10 Plume material tracking equivlent to Asthenosphere = 6
MHE(16,1)=1;                 % shear and adiobatic heating efficiency
...
%upper_cont_thick=40000;

% Lower Continental Crust thickness (m)
%lower_cont_thick = 15000;
% lower_cont_thick=30000;
lower_cont_thick=50000;
...
bot_ccrust1_right = cont_lev + upper_cont_thick_right;
...
contLAB_right = sticky_layer + cont_litho_thick_right;
...
weak_width = 10000;                    % (m)
x1 = cont_ocean + 1000e3;                           % Left Weak zone position (m)
...
% Temperature at bottom of right oceanic lithosphere
...
for mm1 = 1:marknum
    ...
    if(MY(mm1)>cont_lev && MY(mm1)<=bot_ccrust1  && MX(mm1)<=cont_shelf)
        MI(mm1)=6;
    end
    
    % Lower continental crust
    if(MY(mm1)>bot_ccrust1 && MY(mm1)<=bot_ccrust2 && MX(mm1)<=cont_shelf)
        MI(mm1)=7;
    end
    
    % Continetal lab
    if(MY(mm1)>bot_ccrust2 && MY(mm1)<=contLAB && MX(mm1)<=cont_shelf)
        MI(mm1) = 9;
    end
    
    % Upper continental crust of shelf
    shelf_width = cont_ocean - cont_shelf;
    m_ccrust = (bot_ccrust1_right - bot_ccrust1)/shelf_width;
    b_ccrust = bot_ccrust1 - m_ccrust * cont_shelf;
    if(MY(mm1)>cont_lev && MY(mm1)<=(m_ccrust*MX(mm1)+b_ccrust) && MX(mm1)<=cont_ocean && MX(mm1)>cont_shelf)
        MI(mm1)=6;
    end
    
    % Lower continental crust of shelf
    m_ccrust2 = (bot_ccrust2_right - bot_ccrust2)/shelf_width;
    b_ccrust2 = bot_ccrust2 - m_ccrust2 * cont_shelf;
    if(MY(mm1)>(m_ccrust*MX(mm1)+b_ccrust) && MY(mm1)<=(m_ccrust2*MX(mm1)+b_ccrust2) && MX(mm1)<=cont_ocean && MX(mm1)>cont_shelf)
        MI(mm1)=7;
    end
    
%     % Continetal lab of shelf
%     m_contLAB = (contLAB_right - contLAB)/shelf_width;
%     b_contLAB = contLAB - m_contLAB * cont_shelf;
%     if(MY(mm1)>(m_ccrust2*MX(mm1)+b_ccrust2) && MY(mm1)<=(m_contLAB*MX(mm1)+b_contLAB) && MX(mm1)<=cont_ocean && MX(mm1)>cont_shelf)
%         MI(mm1) = 9;
%     end
    
    % arctan version (SUBDUCT20)
    % Continetal lab of shelf
    midpoint_x = (cont_ocean-cont_shelf)/2+cont_shelf;
    if(MY(mm1)>(m_ccrust2*MX(mm1)+b_ccrust2) && MY(mm1)<=(-1*(contLAB-oceanLABsub)/pi()*atan((MX(mm1)-midpoint_x)/10000)+(contLAB-oceanLABsub)/2+oceanLABsub) && MX(mm1)<=cont_ocean && MX(mm1)>cont_shelf)
        MI(mm1) = 9;
    end
...
%     end
    ...
    if( MY(mm1) > cont_lev && MY(mm1) <= contLAB && MX(mm1) <= cont_shelf)
        MTK(mm1) = ttop + (tcontLAB-ttop)*(MY(mm1)-cont_lev)/(contLAB-cont_lev);
    end
    
%     % Continental geotherm of shelf is linear from surface to LAB
%     if(MY(mm1)>cont_lev && MY(mm1)<=(m_contLAB*MX(mm1)+b_contLAB) && MX(mm1)<=cont_ocean && MX(mm1)>cont_shelf)
%         tcontLAB_shelf = tpotential - tabsolute + tgrad*((m_contLAB*MX(mm1)+b_contLAB) - cont_lev);
%         MTK(mm1) = ttop + (tcontLAB_shelf-ttop)*(MY(mm1)-cont_lev)/((m_contLAB*MX(mm1)+b_contLAB)-cont_lev);
%     end
      
    % arctan shelf (SUBDUCT21)
    % Continental geotherm of shelf is linear from surface to LAB
    if(MY(mm1)>cont_lev && MY(mm1)<=(-1*(contLAB-oceanLABsub)/pi()*atan((MX(mm1)-midpoint_x)/10000)+(contLAB-oceanLABsub)/2+oceanLABsub) && MX(mm1)<=cont_ocean && MX(mm1)>cont_shelf)
        tcontLAB_shelf = tpotential - tabsolute + tgrad*((-1*(contLAB-oceanLABsub)/pi()*atan((MX(mm1)-midpoint_x)/10000)+(contLAB-oceanLABsub)/2+oceanLABsub) - cont_lev);
        MTK(mm1) = ttop + (tcontLAB_shelf-ttop)*(MY(mm1)-cont_lev)/((-1*(contLAB-oceanLABsub)/pi()*atan((MX(mm1)-midpoint_x)/10000)+(contLAB-oceanLABsub)/2+oceanLABsub)-cont_lev);
    end
