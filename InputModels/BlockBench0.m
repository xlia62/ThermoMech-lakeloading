...
for mm1 = 1:marknum
    ...
    if (MX(mm1) >= BlockX - BlockWidth/2 && MX(mm1) <= BlockX + BlockWidth/2)
        
        % Sediments
        % Bottom of sediments
        
        bot_osed = sed_thick + sticky_layer + BlockY;
        if (MY(mm1)>sticky_layer + BlockY && MY(mm1)<= bot_osed)
            MI(mm1)=3;
        end
        ...
        % Mantle lithosphere
        if(MY(mm1)>bot_ocrust2 && MY(mm1)<=BlockThick + bot_ocrust2)
            MI(mm1) = 8;
        end
...
for mm1 = 1:marknum   
    ...
    % Block Temperature
    if (MX(mm1) >= BlockX - BlockWidth/2 && MX(mm1) <= BlockX + BlockWidth/2)
        if (MY(mm1)<=BlockThick + bot_ocrust2 && MY(mm1)>=sticky_layer + BlockY)
            MTK(mm1) = BlockT;
        end
    end
