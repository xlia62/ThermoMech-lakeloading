...
MFLOW(8,5)=17;               % Va, cm^3 ===> *1e-6 for J/Pa, or *0.1 for J/bar
                            % uni-axial compression % Lab to model factor
n = MFLOW(8,3);                            
MFLOW(8,6)= 1/2^((n-1)/n)/3^((n+1)/2/n);  
...
MFLOW(10,6) = 1;               % uni-axial compression % Lab to model factor
% n = MFLOW(10,3);                            
% MFLOW(10,6)= 1/2^((n-1)/n)/3^((n+1)/2/n);  
MMU(10,1)=6.7e+10;             % shear modulus, Pa
MPL(10,1)=3e+6;              % C0, Pa
MPL(10,2)=3e+6;              % C1, Pa
MPL(10,3)=0.2;               % sin(FI0)
MPL(10,4)=0.1;               % sin(FI1)
MPL(10,5)=0;                 % GAM0
MPL(10,6)=1;                 % GAM1
...
MFLOW(11,5)=0;               % Va, cm^3 ===> *1e-6 for J/Pa, or *0.1 for J/bar
                            % uni-axial compression % Lab to model factor
n = MFLOW(11,3);                            
MFLOW(11,6)= 1/2^((n-1)/n)/3^((n+1)/2/n);  
MMU(11,1)=6.7e+10;           % shear modulus, Pa
MPL(11,1)=1e+6;              % C0, Pa
MPL(11,2)=1e+6;              % C1, Pa
MPL(11,3)=0;               % sin(FI0)
MPL(11,4)=0;              % sin(FI1)
% MFLOW(11,1)=0;               % 1=power law (wet olivine: Ranalli, 1995)
% MFLOW(11,2)=1e18;          % AD, 1/s/MPa^n                      
% MFLOW(11,6)=1;  
MMU(11,1)=6.7e+10;           % shear modulus, Pa
MPL(11,1)=5e+6;              % C0, Pa
MPL(11,2)=5e+6;              % C1, Pa
MPL(11,3)=0.01;               % sin(FI0)
MPL(11,4)=0.01;              % sin(FI1)
...
MFLOW(12,5)=0;               % Va, cm^3 ===> *1e-6 for J/Pa, or *0.1 for J/bar
                            % uni-axial compression % Lab to model factor
n = MFLOW(12,3);                            
MFLOW(12,6)= 1/2^((n-1)/n)/3^((n+1)/2/n);  
...
MFLOW(13,5)=0;               % Va, cm^3 ===> *1e-6 for J/Pa, or *0.1 for J/bar
                            % uni-axial compression % Lab to model factor
n = MFLOW(13,3);                            
MFLOW(13,6)= 1/2^((n-1)/n)/3^((n+1)/2/n);  
...
MFLOW(14,5)=10;               % Va, cm^3 ===> *1e-6 for J/Pa, or *0.1 for J/bar
                            % uni-axial compression % Lab to model factor
n = MFLOW(14,3);                            
MFLOW(14,6)= 1/2^((n-1)/n)/3^((n+1)/2/n);  
...
MFLOW(15,5)=0;               % Va, cm^3 ===> *1e-6 for J/Pa, or *0.1 for J/bar
                            % uni-axial compression % Lab to model factor
n = MFLOW(15,3);                            
MFLOW(15,6)= 1/2^((n-1)/n)/3^((n+1)/2/n);  
...
% -------------------------------------------------------------------------
% 16 = Perovskite (Keep density same as upper mantle, only change rheology)
%                 (Density increase is handled through the phase function).
% -------------------------------------------------------------------------
MRHO(16,:) = MRHO(10,:);

% Diffusion creep in the lower mantle
MFLOW(16,1) = 1;            % 1=power law ( after Garell et al, 2014)
MFLOW(16,2) = 2.0e-11;      % AD, 1/s/MPa^n
MFLOW(16,3) = 1;            % n
...
% -------------------------------------------------------------------------
% 17 = Perovskite Lithosphere (Only used for tracking, keep same oceanic
%                              lithosphere, change rheology to perovskite)
% -------------------------------------------------------------------------
MRHO(17,:) = MRHO(8,:);
MFLOW(17,:) = MFLOW(16,:);
MMU(17,:) = MMU(8,:);
