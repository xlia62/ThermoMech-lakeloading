...
rplumex = 200e3;
rplumey = 200e3;
xplume = xsize/2 + 250e3; 
yplume = 550e3;  % Will be set to ysize if exceeds ysize
...
% Temperature at bottom of model space will be calculated below using the
% geotherm
...
Tl = 1300;             % Tl = Temperature of the base of Mantle Lithosphere (oC)
...
% LAB depth with sticky_layer is defined as the depth of 1300oC isotherm
k = find(SE.T < 1300-tabsolute,1,'last');
...
%         end
    ...
    if(MY(mm1)>bot_crust2 && MTK(mm1) <= Tl-tabsolute )
        MI(mm1) = 9;
    end
