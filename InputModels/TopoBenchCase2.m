...
MFLOW(6,1)=0;               % 1=power law (wet quartzite: Ranalli, 1995)
MFLOW(6,2)=1e23;          % AD, 1/s/MPa^n
MFLOW(6,3)=2.3;             % n
...
MRHO(4,4)=2400;             % melt density, kg/m^3
MFLOW(7,1)=0;               % 1=power law (plagioclase An75: Ranalli, 1995)
MFLOW(7,2)=1e2;          % AD, 1/s/MPa^n
MFLOW(7,3)=3.2;             % n
...
MFLOW(9,1)= 0;               % 1=power law (dry olivine: Ranalli, 1995)
MFLOW(9,2)= 1e22;          % AD, 1/s/MPa^n
MFLOW(9,3)=3.5;              % n
...
for mm1 = 1:marknum
    ...
    bot_ccrust1 = bot_sed + upper_ccrust_thick;
    if(MY(mm1)>bot_sed && MY(mm1)<=bot_ccrust1 )
        MI(mm1)=6;
    end
    ...
    % Mantle lithosphere
    if(MY(mm1)>bot_ccrust2 && MY(mm1)<=LAB)
        MI(mm1) = 9;
    end
