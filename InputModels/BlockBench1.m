...
RxType =  false;'kinetic'; %'thermodynamic';
...
plate_age = 60e6*yr2sec;  % Subducting
...
% Block Temperature varies linearly from BlockT1 to BlockT2 (left to right)

BlockT1 = 1300 - tabsolute;
BlockT2 = 500 - tabsolute;
...
% Initial block position (m) -- middle of model space
BlockX = xsize0/2;

% Intial upper portion of block postion (m)
BlockY = 80e3;

% Block width (m)
BlockDX = 100e3;

% Block Thickness (Lithosphere thickness) (m)
BlockDY = 200e3;%oceanic_lith_thick;
...
for mm1 = 1:marknum
    ...
    % We create a 90o subduction block with a vertical crust on the right
    if (MY(mm1)>sticky_layer + BlockY && MY(mm1)<= sticky_layer + BlockY + BlockDY)
        
        rightx = BlockX + BlockDX/2;
        
        % Sediments
        % Bottom of sediments
        if (MX(mm1) >=  rightx- sed_thick && MX(mm1) <= rightx)
            MI(mm1)=3;
        end
        
        rightx = rightx - sed_thick;
        % Basaltic crust
        % Bottom of upper oceanic crust
        if (MX(mm1) >= rightx - upper_ocrust_thick && MX(mm1) <= rightx)
            MI(mm1)=4;
        end
        
        rightx = rightx - upper_ocrust_thick;
        % Gabbroic crust
        % Bottom of lower oceanic crust
        if (MX(mm1) >= rightx - lower_ocrust_thick && MX(mm1) <= rightx)
            MI(mm1)=5;
        end
        
        %--------------------------------------------------------
        % The lithospheric block
        %--------------------------------------------------------
        
        rightx = rightx - lower_ocrust_thick;
        % Mantle lithosphere
        if(MX(mm1)>rightx - BlockDX && MX(mm1)<= rightx)
            MI(mm1) = 8;
        end
...
for mm1 = 1:marknum   
    ...
    if( MY(mm1) > sticky_layer && MY(mm1) <= thermLAB)
       z = MY(mm1) - sticky_layer;
       MTK(mm1) = ttop + (MTK(mm1)-ttop).*erf(z/(2*sqrt(kappa*plate_age)));
    end
    
    % Block Temperature
    if (MY(mm1)>sticky_layer + BlockY && MY(mm1)<= sticky_layer + BlockY + BlockDY)
        
        if (MX(mm1) >= BlockX - BlockDX/2 && MX(mm1) <= BlockX + BlockDX/2)
            x = MX(mm1) - (BlockX -BlockDX/2);
            MTK(mm1) = -(BlockT1-BlockT2)*x/BlockDX + BlockT1;            
        end
