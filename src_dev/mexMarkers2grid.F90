...
subroutine mexFunction(nlhs, plhs, nrhs, prhs)
  ...
  double precision, allocatable, dimension(:) :: MSXX, MSXY, MHR, MHE, MTK
  double precision, allocatable, dimension(:) :: MRHOCUR, MRHOCPCUR, METACUR
  double precision, allocatable, dimension(:) :: MHACUR, MMUCUR, MKTCUR
  ...
  type(grid1), dimension(13) ::  Gin, Gout
  ...
  allocate(MSXX(marknum),MSXY(marknum),MHR(marknum),MHE(marknum),MTK(marknum))
  allocate(METACUR(marknum),MMUCUR(marknum),MKTCUR(marknum),MRHOCUR(marknum))
  allocate(MHACUR(marknum),MRHOCPCUR(marknum))

  ! Populate marker arrays: ! MXN,MYN,MDX,MDY,MRHOCUR,MTK,MKTCUR,MRHOCPCUR,MHR,MHE,MHACUR,METACUR,MMUCUR,MSXX,MSXY,G)
  ...
  inmarker_ptr = mxGetPr(prhs(13))
  call mxCopyPtrToReal8(inmarker_ptr,MMUCUR,marknum)
  inmarker_ptr = mxGetPr(prhs(14))
  call mxCopyPtrToReal8(inmarker_ptr,MSXX,marknum)
  inmarker_ptr = mxGetPr(prhs(15))
  call mxCopyPtrToReal8(inmarker_ptr,MSXY,marknum)
   
  ! Extract the fields from the grid structure
  nfields = mxGetNumberOfFields(prhs(16))

  Gin(1)%ptr = mxGetField(prhs(16), 1,'rho1')
  Gin(2)%ptr = mxGetField(prhs(16), 1,'tk1')
  Gin(3)%ptr = mxGetField(prhs(16), 1,'kt1')
  Gin(4)%ptr = mxGetField(prhs(16), 1,'rhocp1')
  Gin(5)%ptr = mxGetField(prhs(16), 1,'hr1')
  Gin(6)%ptr = mxGetField(prhs(16), 1,'he')
  Gin(7)%ptr = mxGetField(prhs(16), 1,'ha1')
  Gin(8)%ptr = mxGetField(prhs(16), 1,'etas1')
  Gin(9)%ptr = mxGetField(prhs(16), 1,'mus1')
  Gin(10)%ptr = mxGetField(prhs(16), 1,'sxy1')
  Gin(11)%ptr = mxGetField(prhs(16), 1,'etan1')
  Gin(12)%ptr = mxGetField(prhs(16), 1,'mun1')
  Gin(13)%ptr = mxGetField(prhs(16), 1,'sxx1')

  ! Create structue for the return argument

  plhs(1) = mxCreateStructArray(1,1,1,mxGetFieldNameByNumber(prhs(16), 1))

  do i = 2,nfields
    k = mxAddField(plhs(1),mxGetFieldNameByNumber(prhs(16), i))
  end do
  ...
  do k = 11,13
    allocate(Gin(k)%prop(ynum-1,xnum-1))
    allocate(Gout(k)%prop(ynum-1,xnum-1))
  end do
  ...
  do k = 11,13
    call mxCopyPtrToReal8(mxGetPr(Gin(k)%ptr),Gin(k)%prop,numelc)
  end do
  ...
  do mm1=1,marknum
    ...
    if (dx<=0.5 .and. dy<=0.5) then
      ...
      Gout(9)%prop(yn,xn)=Gout(9)%prop(yn,xn)+(1.0-dx)*(1.0-dy)*MMUCUR(mm1)*mwt
      Gout(10)%prop(yn,xn)=Gout(10)%prop(yn,xn)+(1.0-dx)*(1.0-dy)*MSXY(mm1)*mwt
      wtetas(yn,xn)=wtetas(yn,xn)+(1.0-dx)*(1.0-dy)*mwt
    end if
    if (dx<=0.5 .and. dy>=0.5) then
      Gout(8)%prop(yn+1,xn)=Gout(8)%prop(yn+1,xn)+(1.0-dx)*dy*METACUR(mm1)*mwt
      Gout(9)%prop(yn+1,xn)=Gout(9)%prop(yn+1,xn)+(1.0-dx)*dy*MMUCUR(mm1)*mwt
      Gout(10)%prop(yn+1,xn)=Gout(10)%prop(yn+1,xn)+(1.0-dx)*dy*MSXY(mm1)*mwt
      wtetas(yn+1,xn)=wtetas(yn+1,xn)+(1.0-dx)*dy*mwt
    end if
    if (dx>=0.5 .and. dy<=0.5) then
      Gout(8)%prop(yn,xn+1)=Gout(8)%prop(yn,xn+1)+dx*(1.0-dy)*METACUR(mm1)*mwt
      Gout(9)%prop(yn,xn+1)=Gout(9)%prop(yn,xn+1)+dx*(1.0-dy)*MMUCUR(mm1)*mwt
      Gout(10)%prop(yn,xn+1)=Gout(10)%prop(yn,xn+1)+dx*(1.0-dy)*MSXY(mm1)*mwt
      wtetas(yn,xn+1)=wtetas(yn,xn+1)+dx*(1.0-dy)*mwt
    end if
    if (dx>=0.5 .and. dy>=0.5) then
      Gout(8)%prop(yn+1,xn+1)=Gout(8)%prop(yn+1,xn+1)+dx*dy*METACUR(mm1)*mwt
      Gout(9)%prop(yn+1,xn+1)=Gout(9)%prop(yn+1,xn+1)+dx*dy*MMUCUR(mm1)*mwt
      Gout(10)%prop(yn+1,xn+1)=Gout(10)%prop(yn+1,xn+1)+dx*dy*MSXY(mm1)*mwt
    ...
    Gout(11)%prop(yn,xn)=Gout(11)%prop(yn,xn)+(1.0-dabs(0.5-dx))*(1.0-dabs(0.5-dy))*METACUR(mm1)*mwt
    Gout(12)%prop(yn,xn)=Gout(12)%prop(yn,xn)+(1.0-dabs(0.5-dx))*(1.0-dabs(0.5-dy))*MMUCUR(mm1)*mwt
    Gout(13)%prop(yn,xn)=Gout(13)%prop(yn,xn)+(1.0-dabs(0.5-dx))*(1.0-dabs(0.5-dy))*MSXX(mm1)*mwt
  ...
  where (wtetas > 0.0d0)
    ...
    Gout(9)%prop=wtetas/Gout(9)%prop
    Gout(10)%prop=Gout(10)%prop/wtetas
  elsewhere
    ! If no new value is interpolated from markers old value is used
    Gout(8)%prop=Gin(8)%prop
    Gout(9)%prop=Gin(9)%prop
    Gout(10)%prop=Gin(10)%prop
  end where
      
  ! Normal viscosity
  where (wtetan > 0.0d0)
    ! Compute new value interpolated from markers
    Gout(11)%prop=Gout(11)%prop/wtetan
    Gout(12)%prop=wtetan/Gout(12)%prop
    Gout(13)%prop=Gout(13)%prop/wtetan
  elsewhere
    ! If no new value is interpolated from markers old value is used
    Gout(11)%prop=Gin(11)%prop
    Gout(12)%prop=Gin(12)%prop
    Gout(13)%prop=Gin(13)%prop
  end where
      
  ! Populate the Output Structure
  call mxSetField(plhs(1),1,'xsize',mxDuplicateArray(mxGetField(prhs(16),1,'xsize')))
  call mxSetField(plhs(1),1,'ysize',mxDuplicateArray(mxGetField(prhs(16),1,'ysize')))
  call mxSetField(plhs(1),1,'xnum',mxDuplicateArray(mxGetField(prhs(16),1,'xnum')))
  call mxSetField(plhs(1),1,'ynum',mxDuplicateArray(mxGetField(prhs(16),1,'ynum')))
  call mxSetField(plhs(1),1,'bx',mxDuplicateArray(mxGetField(prhs(16),1,'bx')))
  call mxSetField(plhs(1),1,'by',mxDuplicateArray(mxGetField(prhs(16),1,'by')))
  call mxSetField(plhs(1),1,'wx',mxDuplicateArray(mxGetField(prhs(16),1,'wx')))
  call mxSetField(plhs(1),1,'wy',mxDuplicateArray(mxGetField(prhs(16),1,'wy')))
  call mxSetField(plhs(1),1,'f',mxDuplicateArray(mxGetField(prhs(16),1,'f')))
  call mxSetField(plhs(1),1,'gridx',mxDuplicateArray(mxGetField(prhs(16),1,'gridx')))
  call mxSetField(plhs(1),1,'gridy',mxDuplicateArray(mxGetField(prhs(16),1,'gridy')))
  call mxSetField(plhs(1),1,'tk2',mxDuplicateArray(mxGetField(prhs(16),1,'tk2')))
  ...
  call mxCopyReal8ToPtr(Gout(9)%prop,mxGetPr(Gout(9)%ptr),numel)
  call mxSetField(plhs(1),1,'mus1',Gout(9)%ptr)
  call mxCopyReal8ToPtr(Gout(10)%prop,mxGetPr(Gout(10)%ptr),numel)
  call mxSetField(plhs(1),1,'sxy1',Gout(10)%ptr)

  do i = 11,13
    Gout(i)%ptr = mxCreateDoubleMatrix(ynum-1,xnum-1,0.d0)
  end do

  call mxCopyReal8ToPtr(Gout(11)%prop,mxGetPr(Gout(11)%ptr),numelc)
  call mxSetField(plhs(1),1,'etan1',Gout(11)%ptr)
  call mxCopyReal8ToPtr(Gout(12)%prop,mxGetPr(Gout(12)%ptr),numelc)
  call mxSetField(plhs(1),1,'mun1',Gout(12)%ptr)
  call mxCopyReal8ToPtr(Gout(13)%prop,mxGetPr(Gout(13)%ptr),numelc)
  call mxSetField(plhs(1),1,'sxx1',Gout(13)%ptr)
