...
if ~restart
    ...
    G.vx1 = zeros(ynum+1,xnum);       % velocity m/s
    G.vy1 = zeros(ynum,xnum+1);       % velocity m/s
...
%--------------------------------------------------------
%% @Liang, Add water loading change 
%-------------------------------------------------------- 
    % time of lake level starting to change 
    % make artificial sediment
    
    k4=(MI==2)&(MY>21.1e3); % when the lake deeper than 1100m 
    MI(k4) = 3;   % Change marker type to sediment
    %MRAT(k) = 1; % Reset strain rate ratio
    %MGII(k) = 0; % Reset strain
    % water_lev_beg_t % second

    T_Ma = timesum/yr2sec/1e6; % My
    
    if lake_level_change 
            % note that water level would change after buffer time
            
        % Sinusoidal curve in certain peroids    
        if T_Ma >= buffer_time + 1e-6*water_lev_beg_t/yr2sec && T_Ma <= buffer_time + 1e-6*(water_lev_beg_t/yr2sec + period*1e3*numer_of_p)
            % the time since lake level changes start
            t_since_change = (timesum - water_lev_beg_t )/yr2sec - buffer_time*1e6; % yr
            % note that the initial water level is at 20e3 m 
            water_lev = ini_water_lev - 0.5*(water_lev_bot - ini_water_lev) * (cos(t_since_change*2*pi/(period*1e3))-1);
            disp(['water level: ', num2str(water_lev), ' m', ' Factor: ', num2str(cos(t_since_change*2*pi/(period*1e3))-1), ...
                ' time since change ', num2str(t_since_change/1e3), ' kyr']) 
        end
        
%         % just drop/rise of lake in certain peroids
%         lake_drop_time = 100e3; %yr
%         if T_Ma >= buffer_time + 1e-6*water_lev_beg_t/yr2sec && T_Ma <=buffer_time + 1e-6*(water_lev_beg_t/yr2sec + lake_drop_time)
%             % the time since lake level changes start in yr
%             t_since_change = (timesum - water_lev_beg_t )/yr2sec - buffer_time*1e6; % yr
%             % note that the initial water level is at 20e3 m 
%             water_lev = (water_lev_bot - ini_water_lev)*t_since_change/lake_drop_time +ini_water_lev;
%             disp(['water level: ', num2str(water_lev), ' m', ...
%              ' time since change ', num2str(t_since_change/1e3), ' kyr']); 
%         end
%         
%         if T_Ma > buffer_time + 1e-6*(water_lev_beg_t/yr2sec + lake_drop_time)
%             water_lev = water_lev_bot;
%             disp(['water level: ', num2str(water_lev)]);
%         end
%         
    ...
    if sticky_layer > 0
        ...
        k = MI > 1 & MI~=2 & MI~=3 & MY < dy; % note act sed is not in topo
...
%         MTK(k) = ttop;
    ...
    if (melting && ntimestep>1) % For best results start at 2, for comparison at 0
        ...
        [XMELT,~] = Melt_fraction_LX(MPR,MTK,MI); % updated case number
        ...
        if extract_melt
            ...
            % surface
            % k1 = MXM > 0;
            k1 = (MXM > 0)&(MI ~=4) & (MI~=6) & (MI~= 11);% Liang added, no crust melting
            ...
            k2 = (MXM > meltmax)&(MI ~=4) & (MI~=6) & (MI~= 11);% Liang added, no crust melting
        ...
        if any(k1)
            ...
            [~, hlat0] = Melt_fraction_LX(MPR(k1)-dp,MTK(k1),MI(k1)); % Liang 
            [~, hlat1] = Melt_fraction_LX(MPR(k1)+dp,MTK(k1),MI(k1));
            MHACUR(k1) = MHACUR(k1) - (hlat1-hlat0)/(2.0*dp);
            
            % Melting heat capacity term: cpm=dHlat/dT
            % Numerical differentiation
            dt = 1.0; % Temperature increment, K
            [~, hlat0] = Melt_fraction_LX(MPR(k1),MTK(k1)-dt,MI(k1));
            [~, hlat1] = Melt_fraction_LX(MPR(k1),MTK(k1)+dt,MI(k1));
...
%end
     ...
     % Multipy effective viscosity by factor F (could be a constant or
     % related to geometric factor following Chpater 6 in Gerya 2010.
     META = META.*MFLOW(MI,6);
    ...
    disp(['Max velocity: ',num2str(max([vxmax(:);vymax(:)])*100*yr2sec),' cm/yr']) % vxmax is in m/s
...
%         MBII(k) = 0; % Reser bulk strain
        ...
        % ensuring a water_level@  change the water level manually 
...
%         end
    ...
    if ntimestep==1  % Safe intial model
        ...
        if RxType
            save([outpath,'/markers_',num2str(out_countb),'.mat'],...
                'MTK','MI','MX','MY','MRHO','MRHOCUR','MFLOW','MMU','MPL','MCP','MKT','MHR','MXN','MXM',...
                'MYN','MSXX','MSXY','META','MEXX','MEXY','MEXT','MEXTC','MPR','MGII','MBII','MRAT','MHE',...
                'MLAMBDA','MECL','MPH410','MPH660','MCXN', 'MCYN', 'TXN', 'plastic_yield','marknum','timesum', 'ntimestep');
        else
            save([outpath,'/markers_',num2str(out_countb),'.mat'],...
            'MTK','MI','MX','MY','MRHO','MRHOCUR','MFLOW','MMU','MPL','MCP','MKT','MHR','MXN','MXM',...
            'MYN','MSXX','MSXY','META','MEXX','MEXY','MEXT','MEXTC','MPR','MGII','MBII','MRAT','MHE',...
        ...
        save_markersbox_lake(outpath,k,MTK,MI,MX,MY,MRHO,MFLOW,MMU,MPL,MCP,MKT,MHR,...
            MSXX,MSXY,META,MEXX,MEXY,MPR,MGII,MBII,MRAT,MCXN, MCYN, TXN, plastic_yield, out_counts);
    ...
    % reset big/small time for restart @ Liang
    if restart 
        time_outb = timesum + Par.yfreq_big;    % full size (big)
        time_outs = timesum + Par.yfreq_small;  % zoomed-in version (small)
...
% Liang's write small    
    if  write_small
        % Zoomed-in 
        if RxType
            save([outpath,'/markers_',num2str(out_counts),'.mat'],...
                'MTK','MI','MX','MY','MRHO','MRHOCUR','MFLOW','MMU','MPL','MCP','MKT','MHR','MXN','MXM',...
                'MYN','MSXX','MSXY','META','MEXX','MEXY','MEXT','MEXTC','MPR','MGII','MBII','MRAT','MHE',...
                'MLAMBDA','MECL','MPH410','MPH660','MCXN', 'MCYN', 'TXN', 'plastic_yield','marknum','timesum', 'ntimestep');
        else
        ...
        save([outpath,'/grids_',num2str(out_counts),'.mat'],...
        'G','gridt','H','E','FastScape','timesum','water_lev','comp_time');
    end
...
if RxType
    ...
    'MTK','MI','MX','MY','MRHO','MRHOCUR','MFLOW','MMU','MPL','MCP','MKT','MHR','MXN','MXM',...
    'MYN','MSXX','MSXY','META','MEXX','MEXY','MEXT','MEXTC','MPR','MGII','MBII','MRAT','MHE',...
    'MLAMBDA','MCXN', 'MCYN', 'TXN', 'plastic_yield','marknum','timesum', 'ntimestep');
end
