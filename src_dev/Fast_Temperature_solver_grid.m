...
function[tknew,rest]=Fast_Temperature_solver_grid(timestep,xnum,ynum,gridx,gridy,kt,rhocp,tk,RT,bleft,bright,btop,bbottom,useSuiteSparse)
% 
...
for i=1:1:ynum
    for j=1:1:xnum
        ...
        % Temperature equation RHO*CP*DT/Dt=-dqx/dx-dqy/dy+Ht
            ...
            I(c) = itk;
            J(c) = itk;
            L(c) = rhocp(i,j)/timestep+((kt(i,j-1)+kt(i,j))/xstp(j-1)+(kt(i,j)+kt(i,j+1))/xstp(j))/2/xstpc(j-1)+((kt(i-1,j)+kt(i,j))/ystp(i-1)+(kt(i,j)+kt(i+1,j))/ystp(i))/2/ystpc(i-1);
            ...
            I(c) = itk;
            J(c) = itk-ynum;
            L(c) = -(kt(i,j-1)+kt(i,j))/2/xstp(j-1)/xstpc(j-1);
            %L(itk,itk-ynum)=-(kt(i,j-1)+kt(i,j))/2/xstp(j-1)/xstpc(j-1);
            % Right T node
            c = c + 1;
            I(c) = itk;
            J(c) = itk+ynum;
            L(c) = -(kt(i,j)+kt(i,j+1))/2/xstp(j)/xstpc(j-1);
            %L(itk,itk+ynum)=-(kt(i,j)+kt(i,j+1))/2/xstp(j)/xstpc(j-1);
            % Upper T node
            c = c + 1;
            I(c) = itk;
            J(c) = itk-1;
            L(c) = -(kt(i-1,j)+kt(i,j))/2/ystp(i-1)/ystpc(i-1);
            %L(itk,itk-1)=-(kt(i-1,j)+kt(i,j))/2/ystp(i-1)/ystpc(i-1);
            % Lower T node
            c = c + 1;
            I(c) = itk;
            J(c) = itk+1;
            L(c) = -(kt(i,j)+kt(i+1,j))/2/ystp(i)/ystpc(i-1);
...
if useSuiteSparse
    L = sparse2(I(1:c),J(1:c),L(1:c),xnum*ynum,xnum*ynum);
else
    L = sparse(I(1:c),J(1:c),L(1:c),xnum*ynum,xnum*ynum);
end

% Solve matrix
S=L\R;

tknew = zeros(ynum,xnum);
% Reload solution
for i=1:1:ynum
    for j=1:1:xnum
        % Index for T
